...
static void TCPServer_WebSocket_DoHandshake(TCPServer_Client* client, SAL_Socket* socket) {
	...
	client->BytesReceived = (uint16)SAL_Socket_Read(socket, client->Buffer + client->BytesReceived, MESSAGE_MAXSIZE - client->BytesReceived);
...
static void TCPServer_WebSocket_OnReceive(TCPServer_Client* client, SAL_Socket* socket) {
	...
	client->BytesReceived += (uint16)received;
			...
			switch (opCode) { //ping is handled by the application, not websocket
				...
				case WS_PING_OPCODE: 
					if (length <= 125) {
						...
						if (SAL_Socket_EnsureWrite(client->Socket, bytes, 2, 10) != 2U || SAL_Socket_EnsureWrite(client->Socket, dataBuffer + headerEnd, length + 4, 10) != length + 4U) {
							TCPServer_DisconnectClient(client);
							return;
						} 
						headerEnd += mask ? WS_MASK_BYTES : 0;
				...
				case WS_BINARY_OPCODE:  
					...
					if (FIN) {
						if (*(uint16*)client->Buffer == client->MessageLength + length - MESSAGE_LENGTHBYTES) {
							client->Server->ReceiveCallback(client, client->State, client->Buffer + MESSAGE_LENGTHBYTES, (uint16)client->MessageLength + length - MESSAGE_LENGTHBYTES);
						}
