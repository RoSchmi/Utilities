...
namespace util {
	...
	namespace crypto {
		 ...
		 */
		std::array<uint8, crypto::sha2_length> calculate_sha2(const uint8* source, word length);

		/**
		 * Take SHA1 of @a length bytes from @a source
		 */
		std::array<uint8, crypto::sha1_length> calculate_sha1(const uint8* source, word length);

		/**
		 * Write @a count random bytes to @a buffer
		 */
		void random_bytes(uint8* buffer, word count);

		/**
		 * @returns a random 64-bit integer in the interval of [@a floor, @a
		 * ceiling]
		 */
		int64 random_int64(int64 floor, int64 ceiling);

		/**
		* @returns a random unsigned 64-bit integer in the interval of [@a floor, @a
		* ceiling]
		*/
		uint64 random_uint64(uint64 floor, uint64 ceiling);

		/**
		 * @returns a random 64-bit floating point number in the interval of [@a floor, @a
		 * ceiling]
		 */
		float64 random_float64(float64 floor, float64 ceiling);
	}
}
